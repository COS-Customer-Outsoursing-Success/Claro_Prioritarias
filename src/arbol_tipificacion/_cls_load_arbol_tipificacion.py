    ...
    def read_arbol(self):
        ...
        try:
            ...
            if latest_file_path is None:
                print("Error: No se puede determinar el archivo más reciente")
                return None
            ...
            self.df = self.df.replace("NA", "NA")  # fuerza que 'NA' sea string
            self.df = self.df.where(pd.notnull(self.df), None)  # convierte NaN a None
            self.df['status'] = self.df['status'].fillna("NA")  # reemplaza NULLs en status

            print("Registros con status 'NA':", (self.df['status'] == "NA").sum())
